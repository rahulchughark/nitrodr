...
if ($_POST['r_email']) {
    ...
    // //$mail->AddAddress("isha.mittal@arkinfo.in", "Isha Mittal");

    // $mail->Subject = "[RAW] - New Raw Lead Added to DR Portal from " . $partner_name . " (" . $_POST['r_user'] . ")";
    // $mail->Body    = "Hi,<br><br> There is new raw lead added to SketchUp DR Portal with details as below:-<br><br>
    // <ul>
    ...
    // $mail->Send();
    if ($res) {
        redir("intern_leads.php?add=success", true);
    } else {
        redir("intern_leads.php?fail=1", true);
    }
}
