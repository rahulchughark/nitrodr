...
if ($_POST['name']) {
    ...
    $res = db_query("INSERT INTO `tbl_product`(`product_name`, `description`, `status`, `created_at`) VALUES ('" . $_POST['name'] . "' ,'" . $_POST['desc'] . "' ,'" . $_POST['status'] . "' ,now())");

    //addProducts('tbl_product',$_POST['name'],$_POST['desc'],$_POST['status'],now());

    $last_insert_id = mysqli_insert_id($GLOBALS['dbcon']);
    $select_query = db_query("select * from tbl_product where id=" . $last_insert_id);
    $result = db_fetch_array($select_query);
    if ($result['product_name'] == 'Parallel') {
        $res = db_query("update tbl_product set form_id = 1 where id=" . $last_insert_id);
    }

    if ($res) {
        redir("manage_products.php?add=success", true);
    }
}
                        ...
                        <div class="card-body">
                            <div class="media bredcrum-title">
                                ...
                                <div class="media-body">
                                    ...
                                    <small class="text-muted">Home > Add Product</small>
                                    <h4 class="font-size-14 m-0 mt-1">Add Product</h4>
                                </div>
                            </div>
                                        ...
                                        <div class="col-lg-4 mb-3">
                                            <div class="form-group row">
                                                ...
                                                <div class="col-md-9 controls">
                                                    <input type="text" name="name" required data-validation-required-message="This field is required" class="form-control" placeholder="Product Name" data-validation-required-message="This field is required">
                                                </div>
                                            </div>
                                        </div>
