...
$setSubject = "Renewal Monthly Visibility Report- ".$data['name']."- ".date('F');	
   
 $quote=getSingleresult("select count(orders.id) as leads from orders left join tbl_lead_product on orders.id =tbl_lead_product.lead_id where orders.team_id='".$data['id']."' and orders.stage='Quote' and license_type='Renewal' and orders.status='Approved' and (tbl_lead_product.product_type_id in (6,7) or tbl_lead_product.product_id is NULL) and date(orders.partner_close_date) BETWEEN  '".$dat1."' and '".$dat2."' ");
//
$verification=getSingleresult("select count(orders.id) as leads from orders left join tbl_lead_product on orders.id =tbl_lead_product.lead_id where orders.team_id='".$data['id']."' and license_type='Renewal' and orders.status='Approved' and (tbl_lead_product.product_type_id in (6,7) or tbl_lead_product.product_id is NULL) and (orders.stage='Verification' or orders.stage='Follow-Up') and date(orders.partner_close_date) BETWEEN  '".$dat1."' and '".$dat2."' ");
//
$commit=getSingleresult("select count(orders.id) as leads from orders left join tbl_lead_product on orders.id =tbl_lead_product.lead_id where orders.team_id='".$data['id']."' and license_type='Renewal' and orders.status='Approved' and (tbl_lead_product.product_type_id in (6,7) or tbl_lead_product.product_id is NULL) and orders.stage='Commit' and date(orders.partner_close_date) BETWEEN  '".$dat1."' and '".$dat2."' ");
//
$booking=getSingleresult("select count(orders.id) as leads from orders left join tbl_lead_product on orders.id =tbl_lead_product.lead_id where orders.team_id='".$data['id']."' and license_type='Renewal' and orders.status='Approved' and (tbl_lead_product.product_type_id in (6,7) or tbl_lead_product.product_id is NULL) and orders.stage='Booking' and date(orders.partner_close_date) BETWEEN  '".$dat1."' and '".$dat2."' ");
//
$eupo=getSingleresult("select count(orders.id) as leads from orders left join tbl_lead_product on orders.id =tbl_lead_product.lead_id where orders.team_id='".$data['id']."' and license_type='Renewal' and orders.status='Approved' and (tbl_lead_product.product_type_id in (6,7) or tbl_lead_product.product_id is NULL) and orders.stage='EU PO Issued' and date(orders.partner_close_date) BETWEEN  '".$dat1."' and '".$dat2."' ");
//
$oem=getSingleresult("select count(orders.id) as leads from orders left join tbl_lead_product on orders.id =tbl_lead_product.lead_id where orders.team_id='".$data['id']."' and orders.stage='OEM Billing' and license_type='Renewal' and orders.status='Approved' and (tbl_lead_product.product_type_id in (6,7) or tbl_lead_product.product_id is NULL) and date(orders.partner_close_date) BETWEEN  '".$dat1."' and '".$dat2."' ");
//
$bor=getSingleresult("select count(orders.id) as leads from orders left join tbl_lead_product on orders.id =tbl_lead_product.lead_id where orders.team_id='".$data['id']."' and orders.stage='Billed To Other Re-Seller' and license_type='Renewal' and orders.status='Approved' and (tbl_lead_product.product_type_id in (6,7) or tbl_lead_product.product_id is NULL) and date(orders.partner_close_date) BETWEEN  '".$dat1."' and '".$dat2."'");
$total_acc=$commit+$booking+$oem+$eupo+$bor;


$quote_quantity=getSingleresult("select COALESCE(SUM(orders.quantity),0) as leads from orders left join tbl_lead_product on orders.id =tbl_lead_product.lead_id where orders.team_id='".$data['id']."' and license_type='Renewal' and orders.status='Approved' and (tbl_lead_product.product_type_id in (6,7) or tbl_lead_product.product_id is NULL) and orders.stage='Quote' and orders.license_type='Renewal' and date(orders.partner_close_date) BETWEEN  '".$dat1."' and '".$dat2."' ");
//
...
//
$oem_quantity=getSingleresult("select COALESCE(SUM(quantity),0) as leads from orders left join tbl_lead_product on orders.id =tbl_lead_product.lead_id where orders.team_id='".$data['id']."'  and license_type='Renewal' and orders.status='Approved' and (tbl_lead_product.product_type_id in (6,7) or tbl_lead_product.product_id is NULL) and orders.stage='OEM Billing'  and date(orders.partner_close_date) BETWEEN  '".$dat1."' and '".$dat2."' ");
...
.c_yellow td{ background-color:#FFFF00;color:#000}
		...
		<p>'.$data['name'].' your Renewal biz monthly visibility is <strong><u>'.$total_quantity.' Seats</u></strong></p>
...
$users1=db_query("select users.email from callers left join orders on orders.caller = callers.id left join users on users.id = callers.user_id where orders.team_id='".$data['id']."' and users.status='Active' and orders.license_type='Renewal'and orders.status='Approved' and orders.stage in ('Commit','EU PO Issued','Booking','OEM Billing','Billed To Other Re-Seller') and date(orders.partner_close_date) BETWEEN  '".$dat1."' and '".$dat2."' GROUP BY orders.caller ");
