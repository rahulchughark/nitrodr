...
<div id="content" class="main-content">
                                        ...
                                        <tbody>
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    @if(empty($programInitiationDate))
                                                                        <!-- <button class="btn btn-small btn-primary" onclick="openProgramModal()">Insert Program Initiation Date</button> -->
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            @php
              $i=0;
                $getGenerateTask=App\GeneratedTask::select("generated_task.mst_task_id","generated_task.task_subject","clm_users.name as updated_by","generated_task.id","generated_task.lead_id","generated_task.task_status","generated_task.task_generate_date","mst_task.task")
                ...
                ->whereNotNull("generated_task.task_owner");
                 if(App\Helpers\AuthHelper::users()->user_type==='FACULTY' || App\Helpers\AuthHelper::users()->user_type==='SALES' || App\Helpers\AuthHelper::users()->user_type==='HELPDESK'){
                    $getGenerateTask->where([["generated_task.lead_id",$lead_id],["generated_task.task_owner",App\Helpers\AuthHelper::users()->id]]);
                 }
                 else if(App\Helpers\AuthHelper::users()->user_type==='ADMIN'){
                    $getGenerateTask->where([["generated_task.lead_id",$lead_id]]);
                 }
                $getAllResult=$getGenerateTask->get();
            @endphp

            @if ($getAllResult->count()>0)
                 @foreach ($getAllResult as $key => $task )
                 <tr>
                    <td>{{++$i}}</td>
                    <td><a data-bs-toggle="modal" onclick="task_model({{$task->mst_task_id}},{{$lead_id}},{{$task->id}})" data-bs-target="#staticBackdrop" style="cursor:pointer">{{$text=$task->task_subject?$task->task_subject:"N/A"}}</a></td>
                    <td>{{$text=$task->task_status?$task->task_status:"N/A"}}</td>
                    
                    <td ><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-calendar"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg> {{$text=$task->task_generate_date?date("d-m-Y",strtotime($task->task_generate_date)):"N/A"}}</td> 
                    <td>{{$text=$task->updated_by?$task->updated_by:""}}</td>
                 ...
                 @else
                 <tr colspan="4">
                    <td class="text-danger">Record not found.!</td>
                 </tr>
  ...
  <table class="table table-bordered">
      <thead>
          ...
          <th style="width:70px">Sr. No.</th>
              ...
              <th scope="col">Negotiate/Sales Price</th>
      ...
      <tbody>
          @php
            $i=0;
            $getProducts=App\LeadProductOpportunity::select("tbl_lead_product_opportunity.id","tbl_lead_product_opportunity.product","tbl_lead_product_opportunity.quantity","tbl_lead_product_opportunity.original_sales_price","tbl_lead_product_opportunity.unit_price","tbl_lead_product_opportunity.total_price","tbl_product_opportunity.product_name")->join("tbl_product_opportunity", "tbl_product_opportunity.id", "=", "tbl_lead_product_opportunity.product")->where("tbl_lead_product_opportunity.status",1)->where("tbl_lead_product_opportunity.lead_id",$lead_id)->get();
        @endphp
          @if ($getProducts->count()>0)
               @foreach ($getProducts as $key => $pro )
               <tr>
                  <td>{{++$i}}</td>
                  <td>{{$pro->product_name}}</td>
                  <td>{{$pro->quantity}}</td>
                  <td>{{$pro->original_sales_price}}</td>
