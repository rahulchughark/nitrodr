...
if ($_GET['id'] != '') {
    $data = db_query("select * from tbl_product where id='" . $_GET['id'] . "'");
...
if (isset($_POST['update_btn'])) {
    ...
    $res = db_query("UPDATE `tbl_product` set `product_name`='" . $_POST['name'] . "', `description`='" . $_POST['desc'] . "', `status`='" . $_POST['status'] . "', `updated_at`=now() where id=" . $_GET['id']);

    //addProducts('tbl_product',$_POST['name'],$_POST['desc'],$_POST['status'],now());

    if ($res) {
        redir("manage_products.php?update=success", true);
    }
}
                                            ...
                                            <div class="col-lg-6 mb-3">
                                                <div class="form-group row">
                                                    ...
                                                    <div class="col-md-9 controls">
                                                        <input type="text" name="name" required data-validation-required-message="This field is required" class="form-control" placeholder="Product Name" data-validation-required-message="This field is required" value="<?= $user_data['product_name'] ?>">
                                                    </div>
                                                </div>
                                            </div>
