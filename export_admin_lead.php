...
if($_GET['d_from'] && $_GET['d_to'])
{
	...
	}else if($_GET['d_type']== 'actioned_date'){
		if($_GET['d_from'] == $_GET['d_to'])
		{
...
if($_GET['school_board'])
{
	$dat.=" and o.school_board in ('".$_GET['school_board']."')";
}
if($_GET['tag'])
{
...
if($_GET['partner'])
{
    $dat.=" and o.team_id in (".$_GET['partner'].")";
}
if($_GET['user'])
{
	$dat.= " and o.created_by in (".$_GET['user'].")";
}

    // $license = $_GET['license'];

       $query = db_query("SELECT o.*,states.name as state,users.name as created_by,callers.name as caller_name,tag.name as tagName,cities.city as cityName FROM orders as o $joinC LEFT JOIN states ON o.state = states.id LEFT JOIN users ON o.created_by = users.id LEFT JOIN callers ON o.caller = callers.id LEFT JOIN cities ON o.city = cities.id LEFT JOIN tag ON o.tag = tag.id where o.is_opportunity=0 AND o.agreement_type = ".$_GET['license']. $dat .$vir_cond." GROUP BY o.id ORDER BY o.id DESC");
...
if($rowCount > 0){
    ...
    while($row = db_fetch_array($query)){
        ...
        $productN =getSingleresult("select tp.product_name from tbl_lead_product as tlp left join tbl_product as tp on tp.id=tlp.product_id where tlp.lead_id=" . $id);
        $subPro = getSingleresult("select tp.product_type from tbl_lead_product as tlp left join tbl_product_pivot as tp on tp.id=tlp.product_type_id where tlp.lead_id=" . $id);
