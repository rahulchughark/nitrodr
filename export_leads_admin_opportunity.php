...
if($_GET['stage'])
{
	$dat.=" and o.stage in ('".implode(",", $_GET['stage'])."')";
}
if($_GET['sub_stage'])
{
	$dat.=" and o.add_comm in ('".implode(",", $_GET['sub_stage'])."')";
}
if($_GET['status'])
{
	$dat.=" and o.status in ('".implode(",", $_GET['status'])."')";
}
if($_GET['school_board'])
{
	$dat.=" and o.school_board in ('".implode(",", $_GET['school_board'])."')";
}
if($_GET['tag'])
{
	$dat.=" and o.tag in (".implode(",", $_GET['tag']).")";
}
if($_GET['state'])
{
	$dat.=" and o.state in (".implode(",", $_GET['state']).")";
}
if($_GET['partner'])
{
    $dat.=" and o.team_id in (".implode(",", $_GET['partner']).")";
}
// if($_GET['user'])
// {
// 	$dat.= " and o.created_by in (".implode(",", $_GET['user']).")";
// }
// $partnerFtr =  $_GET['partner'];
// $usersFtr =  $_GET['users'];
...
if($_GET['source'])
{
	$dat.= " and o.source in ('".implode("','", $_GET['source'])."')";
}

    // $license = $_GET['license'];

       $query = db_query("SELECT o.*,states.name as state,users.name as created_by,callers.name as caller_name,tag.name as tagName,cities.city as cityName FROM orders as o $joinC LEFT JOIN states ON o.state = states.id LEFT JOIN users ON o.created_by = users.id LEFT JOIN callers ON o.caller = callers.id LEFT JOIN cities ON o.city = cities.id LEFT JOIN tag ON o.tag = tag.id where o.is_opportunity=1 AND o.agreement_type = '".$_GET['type']."'". $dat .$vir_cond." GROUP BY o.id  ORDER BY o.created_date desc");
       
//    print_r($query);die;

$rowCount = mysqli_num_rows($query);
if($rowCount > 0){
    $filename = $_GET['type']."_opportunity_" . date('Y-m-d') . ".csv";
