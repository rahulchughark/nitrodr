...
$caller_id = getSingleresult("select id from callers where user_id='" . $_SESSION['user_id'] . "'");


if ($_GET['d_from'] && $_GET['d_to']) {
    if ($_GET['d_from'] == $_GET['d_to']) {

        $cond = " and orders.created_date='" . $_GET['d_from'] . "'";
    } else {
        $cond = " and orders.created_date>='" . $_GET['d_from'] . "' and orders.created_date<='" . $_GET['d_to'] . "' ";
    }
}
if ($_GET['partner']) {
...
$query = db_query("SELECT orders.*, industry.name as industry,sub_ind.name as sub_industry,states.name as state,users.name as created_by FROM orders LEFT JOIN industry ON orders.industry = industry.id LEFT JOIN sub_industry as sub_ind ON orders.sub_industry = sub_ind.id LEFT JOIN states ON orders.state = states.id LEFT JOIN users ON orders.created_by = users.id where orders.license_type='Commercial' and orders.dvr_flag=0 and orders.caller='" . $caller_id . "' $cond ORDER BY id DESC");
