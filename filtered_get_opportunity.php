...
if($product_opp != '' && $product_opp != null && $product_opp != 1)
{
    ...
    $dat.=" and t.main_product_id in (1,2,3)  and t.status=1";
}

$product_opp_type =  json_decode($_REQUEST['product_opp_type']);
$typeForcasting =  json_decode($_REQUEST['type']);
if($product_opp_type != '')
{
    $dat.=" and t.product=".$product_opp_type." and t.status=1";
}
if($productFtr || $product_opp || $product_opp_type || $typeForcasting)
{
	$joinC.= " left join tbl_lead_product_opportunity as t on t.lead_id=o.id";
}else{
	$billClose.=" and o.stage not in ('PO/CIF Issued','Billing')";
}
// getting total number records without any search
// print_r($_REQUEST); die;
$sql = "select o.* ";
$sql.=" FROM orders as o $joinC where o.is_opportunity=1 ".$billClose.$vir_cond.$Type;
...
// 	$createdByCondition = " and created_by =".$_SESSION['user_id'];
// }else{
	$createdByCondition = "";
// }

$sql = "select o.* ";
$sql.=" FROM orders as o $joinC where o.id IN ($getIDs) $createdByCondition";
...
// $sql.=$billClose.$dat.$Type.$vir_cond;
$sql.=$vir_cond;
$sql.=" GROUP BY o.id";
// print_r($sql);die;
// echo $sql;
	...
	$ids2 = "'but2" . $data['id'] . "'";
		   ...
		   $nestedData['close_date'] = ($data['expected_close_date'] ? date('d-m-Y', strtotime($data['expected_close_date'])) : 'N/A') . '<a href="javascript:void(0)" title="Change Close Date" id=but2' . $data['id'] . ' onclick="cd_change(' . $ids2 . ',' . $data['id'] . ')"> <i style="font-size:18px;color:"' . $color . ';font-weight:" . $bold . "" class="mdi mdi-update"></i></a>';
            ...
            while ($subStageHeader = db_fetch_array($sstage_sql)) {
                $nestedData[strtolower(str_replace(' ','_',$subStageHeader['name']))] = $helperData->getDataValues($data['id'],$subStageHeader['name']).'<a  href="javascript:void(0)" title="Change Close Date" 
                id=demo'.$data['id'].$subStageHeader['id'].' onclick="demo_sub_stage_date(' . $data['id'] . ','.$subStageHeader['id'].')"> <i style="font-size:18px;color:"' . $color . ';font-weight:" . $bold . "" class="mdi mdi-update"></i></a>';
            }
