...
if ($requestData['status'] == 'Pending') {
	...
	$sql = "select * from ((select o.*,lm.created_date as converted_date,lm.created_by as converted_by from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join lead_modify_log lm on o.id=lm.lead_id left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and lm.type='Lead Type' and lm.previous_name in ('BD','Incoming') and lm.modify_name='LC' and o.lead_type='LC' and EXISTS( select o.*,lm.created_date as converted_date,lm.created_by as converted_by from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join lead_modify_log lm on o.id=lm.lead_id left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and lm.type='Caller' and lm.previous_name ='' and lm.modify_name in ('" . $iss_names . "')) $dat $dat1 GROUP BY o.id ) UNION ALL (select o.*,lm.created_at as converted_date,lm.sender_id from orders as o left join lead_notification lm on o.id=lm.type_id where lm.title in ('Request BD to LC','Request Incoming to LC') and lm.is_read=0 and lm.sender_type='Partner' $dat group by lm.type_id)) as i group by i.id order by i.id desc";
}
//echo $sql; die;
...
if (!empty($requestData['search']['value'])) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
	...
	if ($requestData['status'] == 'Pending') {
		$sql = "select o.*,lm.created_date as converted_date,lm.created_by as converted_by from orders as o left join lead_notification l on o.id=l.type_id left join lead_modify_log lm on o.id=lm.lead_id  left join industry as i on o.industry=i.id where l.title in ('Request BD to LC','Request Incoming to LC') and l.is_read=0 and l.sender_type='Partner' $dat $search group by l.type_id order by o.id desc";
	} else if($requestData['status'] == 'Completed'){
		$sql = "select * from ((select o.*,lm.created_date as converted_date,lm.created_by as converted_by from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join lead_modify_log lm on o.id=lm.lead_id left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and lm.type='Lead Type' and lm.previous_name in ('BD','Incoming') and lm.modify_name='LC' and o.lead_type='LC' and EXISTS( select o.*,lm.created_date as converted_date,lm.created_by as converted_by from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join lead_modify_log lm on o.id=lm.lead_id left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and lm.type='Caller' and lm.previous_name ='' and lm.modify_name in ('" . $iss_names . "')) $dat $search GROUP BY o.id) UNION ALL (select o.*,lm.created_at as converted_date,lm.sender_id  from orders as o left join lead_notification lm on o.id=lm.type_id where lm.title in ('Request BD to LC','Request Incoming to LC') and lm.is_read=1 and lm.sender_type='Partner' $dat $search group by lm.type_id)) as i group by i.id order by i.id desc";
	...
	if ($requestData['d_from'] == $requestData['d_to']) {
		...
		$sql = "select * from ((select o.*,lm.created_date as converted_date,lm.created_by as converted_by from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join lead_modify_log lm on o.id=lm.lead_id left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and lm.type='Lead Type' and lm.previous_name in ('BD','Incoming') and lm.modify_name='LC' and o.lead_type='LC' and EXISTS( select o.*,lm.created_date as converted_date,lm.created_by as converted_by from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join lead_modify_log lm on o.id=lm.lead_id left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and lm.type='Caller' and lm.previous_name ='' and lm.modify_name in ('" . $iss_names . "')) $dat $search GROUP BY o.id) UNION ALL (select o.*,lm.created_at as converted_date,lm.sender_id  from orders as o left join lead_notification lm on o.id=lm.type_id where lm.title in ('Request BD to LC','Request Incoming to LC') and lm.is_read=0 and lm.sender_type='Partner' $dat $search group by lm.type_id)) as i group by i.id order by i.id desc ";
	}
}
...
while ($data = db_fetch_array($query)) {  // preparing an array
	...
	//getSingleresult("select is_read from lead_notification where title='Request BD to LC' and type_id=" . $data['id']." "" and EXISTS (select * from lead_modify_log where previous_name in ('BD','Incoming') and modify_name='LC')");

	$nestedData[] =  "<a target='_blank' style='display:block;color:" . $color . "' href='view_order.php?id=" . $data['id'] . "'>" . $data['r_name'] . '(' . $data['r_user'] . ')' . '</a>';
	...
	$nestedData[] = "<a target='_blank' style='display:block;color:" . $color . "' href='view_order.php?id=" . $data['id'] . "'>" . date('d-m-Y', strtotime($data['created_date'])) . '</a>';
