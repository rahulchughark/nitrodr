...
$iss_users = db_query("select c.name,c.id from users left join callers as c on users.id=c.user_id where users.user_type='CLR' and users.role='ISS' and users.status='Active' ");
...
if ($_SESSION['user_type'] == 'ISS MNGR' || $_SESSION['user_id'] == '118' || $_SESSION['user_id'] == '122' || $_SESSION['user_id'] == '123' || $_SESSION['user_id'] == '124' || $_SESSION['user_type'] == 'ISS SBE MNGR') {
...
if ($_SESSION['user_type'] == 'ISS MNGR' || $_SESSION['user_id'] == '118' || $_SESSION['user_id'] == '122' || $_SESSION['user_id'] == '123' || $_SESSION['user_id'] == '124') {
    if ($requestData['status'] == 'Pending') {
        ...
        $sql = "select * from ((select o.*,lm.created_date as converted_date,lm.created_by as converted_by from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join lead_modify_log lm on o.id=lm.lead_id left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and lm.type='Lead Type' and lm.previous_name in ('BD','Incoming') and lm.modify_name='LC' and o.lead_type='LC' and EXISTS (select o.*,lm.created_date as converted_date,lm.created_by as converted_by from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join lead_modify_log lm on o.id=lm.lead_id left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and lm.type='Caller' and lm.previous_name ='') $contd $dat GROUP BY o.id) UNION ALL (select o.*,lm.created_at as converted_date,lm.sender_id as converted_by from orders as o left join lead_notification lm on o.id=lm.type_id where lm.title in ('Request BD to LC','Request Incoming to LC') and lm.is_read=0 and lm.sender_type='Partner' $dat $contd group by lm.type_id)) as i group by i.id order by i.id desc";
    }
} else {
...
if (!empty($requestData['search']['value'])) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
    ...
    if ($_SESSION['user_type'] == 'ISS MNGR' || $_SESSION['user_id'] == '118' || $_SESSION['user_id'] == '122' || $_SESSION['user_id'] == '123' || $_SESSION['user_id'] == '124') {
        if ($requestData['status'] == 'Pending') {
            $sql = "select o.*,l.created_date as converted_date,l.created_by as converted_by from orders as o left join lead_notification l on o.id=l.type_id left join lead_modify_log lm on o.id=lm.lead_id  left join industry as i on o.industry=i.id where l.title in ('Request BD to LC','Request Incoming to LC') and l.is_read=0 and l.sender_type='Partner' $dat $contd $search group by l.type_id order by o.id desc";

        } else if ($requestData['status'] == 'Completed') {
            $sql = "select * from ((select o.*,lm.created_date as converted_date,lm.created_by as converted_by from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join lead_modify_log lm on o.id=lm.lead_id left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and lm.type='Lead Type' and lm.previous_name in ('BD','Incoming') and lm.modify_name='LC' and o.lead_type='LC' and EXISTS( select o.*,lm.created_date as converted_date,lm.created_by as converted_by from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join lead_modify_log lm on o.id=lm.lead_id left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and lm.type='Caller' and lm.previous_name ='' and lm.modify_name in ('" . $iss_names . "')) $dat $contd $search GROUP BY o.id) UNION ALL (select o.*,lm.created_at as converted_date,lm.sender_id  from orders as o left join lead_notification lm on o.id=lm.type_id where lm.title in ('Request BD to LC','Request Incoming to LC') and lm.is_read=1 and lm.sender_type='Partner' $dat $contd $search group by lm.type_id)) as i group by i.id order by i.id desc";
            ...
            $sql = "select * from ((select o.*,lm.created_date as converted_date,lm.created_by as converted_by from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join lead_modify_log lm on o.id=lm.lead_id left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and lm.type='Lead Type' and lm.previous_name in ('BD','Incoming') and lm.modify_name='LC' and o.lead_type='LC' and EXISTS (select o.*,lm.created_date as converted_date,lm.created_by as converted_by from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join lead_modify_log lm on o.id=lm.lead_id left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and lm.type='Caller' and lm.previous_name ='') $contd $dat $search GROUP BY o.id) UNION ALL (select o.*,lm.created_at as converted_date,lm.sender_id as converted_by from orders as o left join lead_notification lm on o.id=lm.type_id where lm.title in ('Request BD to LC','Request Incoming to LC') and lm.is_read=0 and lm.sender_type='Partner' $dat $contd $search group by lm.type_id)) as i group by i.id order by i.id desc";
        }
    } else {
        if ($requestData['d_from'] && $requestData['d_to']) {

            if ($requestData['d_from'] == $requestData['d_to']) {
                $sql = "select o.*,lm.created_date as converted_date,lm.created_by as converted_by from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join lead_modify_log lm on o.id=lm.lead_id left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and lm.type='Lead Type' and lm.previous_name in ('BD','Incoming') and lm.modify_name='LC' and o.lead_type='LC' and EXISTS( select o.*,lm.created_date as converted_date,lm.created_by as converted_by from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join lead_modify_log lm on o.id=lm.lead_id left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and lm.type='Caller' and lm.previous_name ='') and DATE(lm.created_date)='" . $requestData['d_from'] . "' $contd $dat $search GROUP BY o.id";
                
            } else {
                $sql = "select o.*,lm.created_date as converted_date,lm.created_by as converted_by from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join lead_modify_log lm on o.id=lm.lead_id left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and lm.type='Lead Type' and lm.previous_name in ('BD','Incoming') and lm.modify_name='LC' and o.lead_type='LC' and EXISTS( select o.*,lm.created_date as converted_date,lm.created_by as converted_by from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join lead_modify_log lm on o.id=lm.lead_id left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and lm.type='Caller' and lm.previous_name ='') and  (date(lm.created_date) BETWEEN  '" . $requestData['d_from'] . "' and '" . $requestData['d_to'] . "') $contd $dat $search GROUP BY o.id";
            }
        }else{
            $sql = "select * from ((select o.*,lm.created_date as converted_date,lm.created_by as converted_by from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join lead_modify_log lm on o.id=lm.lead_id left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and lm.type='Lead Type' and lm.previous_name in ('BD','Incoming') and lm.modify_name='LC' and o.lead_type='LC' and EXISTS( select o.*,lm.created_date as converted_date,lm.created_by as converted_by from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join lead_modify_log lm on o.id=lm.lead_id left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and lm.type='Caller' and lm.previous_name ='') $contd $dat $search GROUP BY o.id) UNION ALL (select o.*,lm.created_at as converted_date,lm.sender_id as converted_by from orders as o left join lead_notification lm on o.id=lm.type_id where lm.title in ('Request BD to LC','Request Incoming to LC') and lm.is_read=0 and lm.sender_type='Partner' $dat $contd $search group by lm.type_id)) as i group by i.id order by i.id desc";
        }
...
while ($data = db_fetch_array($query)) {  // preparing an array
    ...
    $nestedData[] =  "<a target='_blank' style='display:block;color:" . $color . "' href='caller_view.php?id=" . $data['id'] . "'>" . $data['r_name'] . '(' . $data['r_user'] . ')' . '</a>';

    $nestedData[] = "<a target='_blank' style='display:block;color:" . $color . "' href='caller_view.php?id=" . $data['id'] . "'>" . $data['lead_type'] . '</a>';

    $nestedData[] = "<a target='_blank' style='display:block;color:" . $color . "' href='view_order.php?id=" . $data['id'] . "'>" . $data['quantity'] . '</a>';

    $nestedData[] =  "<a target='_blank' style='display:block;color:" . $color . "' href='caller_view.php?id=" . $data['id'] . "'>" . $data['company_name'] . '</a>';

    $nestedData[] = "<a target='_blank' style='display:block;color:" . $color . "' href='caller_view.php?id=" . $data['id'] . "'>" . date('d-m-Y', strtotime($data['created_date'])) . '</a>';
	...
	$lead_status = getSingleresult("select * from lead_modify_log where previous_name in ('BD','Incoming') and modify_name='LC' and type='Lead Type' and lead_id=".$data['id']);
        ...
        $nestedData[] =  date('d-m-Y', strtotime($data['converted_date']));   
        $nestedData[] =  getSingleresult("select name from callers where id='" . $data['caller'] . "'");   
        $nestedData[] = getSingleresult("select name from users where id='" . $data['converted_by'] . "'");
    }
