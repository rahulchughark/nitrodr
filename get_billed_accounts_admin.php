...
$sql = billedAccountsSearchAdmin($dat,$condition);

// echo $sql; die;
$query = db_query($sql);
// when there is no search parameter then total number rows = total number filtered rows.

$sql = billedAccountsLeadAdmin($condition);
...
while ($data = db_fetch_array($query)) {  // preparing an array
	...
	$nestedData['r_user'] = "<a style='display:block;color:" . $color . "' href='billed_account_view_admin.php?id=" . $data['id'] . "'>" . $data['r_user'] . ($data['allign_to'] ? '(' . getSingleresult("select name from users where id=" . $data['allign_to']) . ')' : '') . '</a>';
	...
	$nestedData['lead_type'] = "<a  style='display:block;color:" . $color . "' href='billed_account_view_admin.php?id=" . $data['id'] . "'>" . $data['lead_type'] . '</a>';
	
	$nestedData['quantity'] = "<a style='display:block;color:" . $color . "' href='billed_account_view_admin.php?id=" . $data['id'] . "'>" . $data['quantity'] . '</a>';

	$nestedData['product_name'] = "<a style='display:block;color:" . $color . "' href='billed_account_view_admin.php?id=" . $data['id'] . "'>" . (getSingleresult("select p.product_name from tbl_lead_product as l left join tbl_product as p on l.product_id=p.id where l.lead_id=" . $data['id']) ? (getSingleresult("select p.product_name from tbl_lead_product as l left join tbl_product as p on l.product_id=p.id where l.lead_id=" . $data['id'])) : 'N/A') . '</a>';

	$nestedData['product_type'] = "<a style='display:block;color:" . $color . "' href='billed_account_view_admin.php?id=" . $data['id'] . "'>" . (getSingleresult("select p.product_type from tbl_lead_product as l left join tbl_product_pivot as p on l.product_type_id=p.id where l.lead_id=" . $data['id']) ? (getSingleresult("select p.product_type from tbl_lead_product as l left join tbl_product_pivot as p on l.product_type_id=p.id where l.lead_id=" . $data['id'])) : 'N/A') . '</a>';

	$nestedData['company_name'] = "<a style='display:block;color:" . $color . "' href='billed_account_view_admin.php?id=" . $data['id'] . "'>" . $data['company_name'] . '</a>';
