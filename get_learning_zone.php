...
if (!empty($category)) {
	...
	// $sql = "select learning_zone.*,training_videos.vdo_address FROM learning_zone left join training_videos on learning_zone.video_id=training_videos.id where learning_zone.status=1 and learning_zone.type='Video' ";
    $sql = "
            SELECT DISTINCT lz.*
	...
	if (!empty($requestData['search']['value'])) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
		...
		$sql = "select learning_zone.*,training_videos.vdo_address FROM learning_zone left join training_videos on learning_zone.video_id=training_videos.id where learning_zone.status=1 and learning_zone.type='Video' $where $search ORDER BY id desc";
...
while ($data = db_fetch_array($query)) {  // preparing an array
	...
	$nestedData = array();
	$nestedData[] = $i;
    $nestedData[] = date('d-m-Y',strtotime($data['updated_at']));
   $nestedData[] = '<a title="View File" onclick="return document_view('.$data["id"].')" href="javascript:void(0)" class="btn btn-primary px-2 py-2"><i class="fa fa-folder-open" aria-hidden="true"></i></a>';
    $nestedData[] = !$data['category_id'] ? $data['document_category'] : $dataObj->getCategoryPath($data['category_id']);
    // $nestedData[] = "Category Path";
	unset($partnerName);
	$results[] = $nestedData;
	$i++;
}
