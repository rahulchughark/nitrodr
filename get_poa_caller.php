...
if ($requestData['assigned_by']) {
    $dat .= ' and a.added_by in ("' . stripslashes($requestData["assigned_by"]) . '")';
}
...
$contd .= ' and a.action_plan in ("' . stripslashes($requestData["current_poa"]) . '")';
// getting total number records without any search

if($requestData['current_poa'])
{   
    $curr_poa = str_replace("Completed","",$requestData["current_poa"]);
    $dat2 = $dat;
    $dat2 .= ' and a.action_plan in ("' . stripslashes($curr_poa) . '")';
  ...
  while ($data = db_fetch_array($query)) {
    $apid[] = getSingleresult("select id from activity_log where is_intern=0 and pid='" . $data['id'] . "'  ORDER BY id DESC limit 1");
  }

  $apid = implode(',',$apid);

  $sql = "select o.*,a.action_plan,a.created_date as activity_date,a.added_by as assigned_by,c.name as caller from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join activity_log a on o.id=a.pid left join callers c on c.id=o.caller left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and o.status='Approved' and a.is_intern=0 $dat2 and a.id in (".$apid.") order by a.created_date desc";

    // if($requestData['current_poa']!='Completed'){
    //     $sql = "select o.*,a.action_plan,a.created_date as activity_date,a.added_by as assigned_by,c.name as caller from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join activity_log a on o.id=a.pid left join callers c on c.id=o.caller left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and o.status='Approved' and a.is_intern=0 and a.action_plan!='' $contd $dat group by a.pid having (count(a.pid)>1) order by a.created_date desc";
    // }else{
    //     $sql = "select o.*,a.action_plan,a.created_date as activity_date,a.added_by as assigned_by,c.name as caller from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join activity_log a on o.id=a.pid left join callers c on c.id=o.caller left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and o.status='Approved' and a.is_intern=0 and a.action_plan='' $dat  group by a.pid having (count(a.pid)>1) order by a.created_date desc";
    // }
}else{
...
if($requestData['current_poa'])
{
  ...
  $sql = "select o.*,a.action_plan,a.created_date as activity_date,a.added_by as assigned_by,c.name as caller from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join activity_log a on o.id=a.pid left join callers c on c.id=o.caller left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and o.status='Approved' and a.is_intern=0 $dat2 $search and a.id in(".$apid.") order by a.created_date desc";
}else{
    $sql = "select o.*,a.action_plan,a.created_date as activity_date,a.added_by as assigned_by,c.name as caller from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join activity_log a on o.id=a.pid left join callers c on c.id=o.caller left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and o.status='Approved' and a.is_intern=0 and a.action_plan!='' $dat $search group by a.pid having (count(a.pid)>1) order by a.created_date desc";
 }
}
...
while ($data = db_fetch_array($query)) {  // preparing an array
    ...
    $nestedData[] =  "<a target='_blank' style='display:block;color:" . $color . "' href='caller_view.php?id=" . $data['id'] . "'>" . $data['r_name'] . '</a>';
    $nestedData[] =  "<a target='_blank' style='display:block;color:" . $color . "' href='caller_view.php?id=" . $data['id'] . "'>" . $data['r_user'] . '</a>';
    $nestedData[] = "<a target='_blank' style='display:block;color:" . $color . "' href='caller_view.php?id=" . $data['id'] . "'>" . $data['lead_type'] . '</a>';
    $nestedData[] = "<a target='_blank' style='display:block;color:" . $color . "' href='caller_view.php?id=" . $data['id'] . "'>" . $data['quantity'] . '</a>';
    $nestedData[] = "<a target='_blank' style='display:block;color:" . $color . "' href='caller_view.php?id=" . $data['id'] . "'>" . $data['caller'] . '</a>';
    $nestedData[] =  "<a target='_blank' style='display:block;color:" . $color . "' href='caller_view.php?id=" . $data['id'] . "'>" . $data['company_name'] . '</a>';
    $nestedData[] = "<a target='_blank' style='display:block;color:" . $color . "' href='caller_view.php?id=" . $data['id'] . "'>" . date('d-m-Y', strtotime($data['approval_time'])) . '</a>';

    $nestedData[] = date('d-m-Y', strtotime($data['activity_date']));
    if($requestData['current_poa']){
        $nestedData[] = getSingleresult("select action_plan from activity_log where is_intern=0 and pid='" . $data['id'] . "' and action_plan!='' group by pid having (count(pid)>0) order by created_date desc");
    }else{
