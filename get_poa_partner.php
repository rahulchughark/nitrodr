...
if($requestData['current_poa']){
    ...
    $sql = "select o.*,a.action_plan,a.created_date as activity_date,a.added_by as assigned_by,c.name as caller from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join activity_log a on o.id=a.pid left join callers c on c.id=o.caller left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and o.status='Approved' and o.team_id=".$teamId." and a.is_intern=0 and a.action_plan!='' $dat  group by a.pid having (count(a.pid)>0) order by a.created_date desc";
  ...
  $sql = "select o.*,a.action_plan,a.created_date as activity_date,a.added_by as assigned_by,c.name as caller from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join activity_log a on o.id=a.pid left join callers c on c.id=o.caller left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and o.status='Approved' and o.team_id=".$teamId." and a.is_intern=0 $dat2 and a.id in(".$apid.") order by a.created_date desc";
...
if (!empty($requestData['search']['value'])) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
  ...
  $sql = "select o.*,a.action_plan,a.created_date as activity_date,a.added_by as assigned_by,c.name as caller from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join activity_log a on o.id=a.pid left join callers c on c.id=o.caller left join industry as i on o.industry=i.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and o.status='Approved' and o.team_id=".$teamId." and a.is_intern=0 and a.id in(".$apid.") $dat2 $search order by a.created_date desc";
// echo $sql; die;
...
while ($data = db_fetch_array($query)) {  // preparing an array
    ...
    $nestedData[] = "<a target='_blank' style='display:block;color:" . $color . "' href='partner_view.php?id=" . $data['id'] . "'>" . $data['quantity'] . '</a>';
    $nestedData[] = "<a target='_blank' style='display:block;color:" . $color . "' href='partner_view.php?id=" . $data['id'] . "'>" . $data['caller'] . '</a>';
    $nestedData[] =  "<a target='_blank' style='display:block;color:" . $color . "' href='partner_view.php?id=" . $data['id'] . "'>" . $data['company_name'] . '</a>';
    $nestedData[] = "<a target='_blank' style='display:block;color:" . $color . "' href='partner_view.php?id=" . $data['id'] . "'>" . date('d-m-Y', strtotime($data['approval_time'])) . '</a>';
