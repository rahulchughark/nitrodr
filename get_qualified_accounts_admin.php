...
if ($_SESSION['sales_manager'] == 1) {
    $con .= " and o.team_id in (" . $_SESSION['access'] . ") ";
	$con2 .= " and ol.team_id in (" . $_SESSION['access'] . ") ";
}
...
// UNION ALL 
// 	     SELECT ol.id,ol.r_name,ol.r_user,ol.lead_type,ol.company_name,ol.quantity,ol.created_date,ol.created_date,ol.status,i.name,ml.created_date as actioned_date FROM  lapsed_orders as ol left join industry as i on ol.industry=i.id left join tbl_lead_product as tp on ol.id=tp.lead_id left join lead_modify_log as ml on ol.id=ml.lead_id where ol.status='Approved' AND tp.product_id=1 AND tp.product_type_id IN(1,2) AND ml.type='Status' AND ml.modify_name='Approved' order by id DESC

$sql = "SELECT * FROM (SELECT o.id,o.r_name,o.r_user,o.lead_type,o.company_name,o.quantity,o.created_date,o.status,o.close_time,i.name as industry,ml.created_date as actioned_date,ml.type  FROM orders as o left join industry as i on o.industry=i.id join tbl_lead_product as tp on o.id=tp.lead_id join lead_modify_log as ml on o.id=ml.lead_id where o.status='Approved' AND tp.product_id=1 AND tp.product_type_id IN(1,2) $stCon1 $con GROUP by o.id
UNION SELECT o.id,o.r_name,o.r_user,o.lead_type,o.company_name,o.quantity,o.created_date,o.status,o.close_time,i.name as industry,ml.created_date as actioned_date,ml.type FROM orders as o left join industry as i on o.industry=i.id join tbl_lead_product as tp on o.id=tp.lead_id join lead_modify_log as ml on o.id=ml.lead_id where o.status='Approved' AND tp.product_id=1 AND tp.product_type_id IN(1,2) $stCon2 $con GROUP by o.id
UNION SELECT ol.id,ol.r_name,ol.r_user,ol.lead_type,ol.company_name,ol.quantity,ol.created_date,ol.status,ol.close_time,i.name as industry,ml.created_date as actioned_date,ml.type  FROM lapsed_orders as ol left join industry as i on ol.industry=i.id join tbl_lead_product as tp on ol.id=tp.lead_id join lead_modify_log as ml on ol.id=ml.lead_id where ol.status='Approved' AND tp.product_id=1 AND tp.product_type_id IN(1,2) $stCon1 $con2 GROUP by ol.id 
UNION SELECT ol.id,ol.r_name,ol.r_user,ol.lead_type,ol.company_name,ol.quantity,ol.created_date,ol.status,ol.close_time,i.name as industry,ml.created_date as actioned_date,ml.type FROM lapsed_orders as ol left join industry as i on ol.industry=i.id join tbl_lead_product as tp on ol.id=tp.lead_id join lead_modify_log as ml on ol.id=ml.lead_id where ol.status='Approved' AND tp.product_id=1 AND tp.product_type_id IN(1,2) $stCon2 $con2 GROUP by ol.id) as i order by i.id DESC";
...
if (!empty($requestData['search']['value'])) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
	$search = " AND (o.r_name LIKE '%" . $requestData['search']['value'] . "%' 
	 ...
	 $sql = "SELECT * FROM (SELECT o.id,o.r_name,o.r_user,o.lead_type,o.company_name,o.quantity,o.created_date,o.status,o.close_time,i.name as industry,ml.created_date as actioned_date,ml.type  FROM orders as o left join industry as i on o.industry=i.id join tbl_lead_product as tp on o.id=tp.lead_id join lead_modify_log as ml on o.id=ml.lead_id where o.status='Approved' AND tp.product_id=1 AND tp.product_type_id IN(1,2) $search $stCon1 $con GROUP by o.id
	UNION SELECT o.id,o.r_name,o.r_user,o.lead_type,o.company_name,o.quantity,o.created_date,o.status,o.close_time,i.name as industry,ml.created_date as actioned_date,ml.type FROM orders as o left join industry as i on o.industry=i.id join tbl_lead_product as tp on o.id=tp.lead_id join lead_modify_log as ml on o.id=ml.lead_id where o.status='Approved' AND tp.product_id=1 AND tp.product_type_id IN(1,2) $search $stCon2 $con GROUP by o.id
	UNION SELECT ol.id,ol.r_name,ol.r_user,ol.lead_type,ol.company_name,ol.quantity,ol.created_date,ol.status,ol.close_time,i.name as industry,ml.created_date as actioned_date,ml.type  FROM lapsed_orders as ol left join industry as i on ol.industry=i.id join tbl_lead_product as tp on ol.id=tp.lead_id join lead_modify_log as ml on ol.id=ml.lead_id where ol.status='Approved' AND tp.product_id=1 AND tp.product_type_id IN(1,2) $search2 $stCon1 $con2 GROUP by ol.id 
	UNION SELECT ol.id,ol.r_name,ol.r_user,ol.lead_type,ol.company_name,ol.quantity,ol.created_date,ol.status,ol.close_time,i.name as industry,ml.created_date as actioned_date,ml.type FROM lapsed_orders as ol left join industry as i on ol.industry=i.id join tbl_lead_product as tp on ol.id=tp.lead_id join lead_modify_log as ml on ol.id=ml.lead_id where ol.status='Approved' AND tp.product_id=1 AND tp.product_type_id IN(1,2) $search2 $stCon2 $con2 GROUP by ol.id) as i order by i.id DESC";
...
while ($data = db_fetch_array($query)) {  // preparing an array
   ...
   $submission_type = ($data['type'] == 'Status')?'Fresh':'Converted';
	...
	$nestedData['id'] = $i;
	$nestedData['r_name'] =  "<a target='_blank' style='display:block;color:" . $color . "' href='view_order.php?id=" . $data['id'] . "'>" . $data['r_name'] . '(' . $data['r_user'] . ')' . '</a>';
	...
	$nestedData['industry'] = "<a target='_blank' style='display:block;color:" . $color . "' href='view_order.php?id=" . $data['id'] . "'>" . $data['industry'] . '</a>';
	$nestedData['created_date'] = "<a target='_blank' style='display:block;color:" . $color . "' href='view_order.php?id=" . $data['id'] . "'>" . date('d-m-Y',strtotime($data['created_date'])) . '</a>';
	$nestedData['actioned_date'] = "<a target='_blank' style='display:block;color:" . $color . "' href='view_order.php?id=" . $data['id'] . "'>" . date('d-m-Y',strtotime($data['actioned_date'])) . '</a>';
