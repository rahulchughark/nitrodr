...
if($requestData['license_to'] && $requestData['license_from']){
	if ($requestData['license_to'] == $requestData['license_from']) {
		$dat = " and license_end_date='" . $license_from . "'";
	} else {
		$dat = " and (date(license_end_date) between '" . $license_from . "' and '" . $license_to . "')";
	}
	}
...
if ($requestData['ltype']) {
	$dat .= " and license_type='" . $requestData['ltype'] . "'";
}
...
$sql = "select * ";
$sql .= " FROM orders where 1 and  license_type = 'Renewal' and dvr_flag!=1 " . $dat . $vir_cond;
//echo $sql; die;
$query = db_query($sql);
...
$sql = "select * ";
$sql .= " FROM orders WHERE 1=1 and license_type = 'Renewal' and dvr_flag!=1 ";
...
while ($data = db_fetch_array($query)) {  // preparing an array
	...
	$nestedData['r_user'] = "<a style='display:block;color:" . $color . "' href='view_order.php?id=" . $data['id'] . "'>" . $data['r_user'] . '(' . $data['r_user'] . ')</a>';
	$nestedData['license_number'] = "<a  style='display:block;color:" . $color . "' href='view_order.php?id=" . $data['id'] . "'>" . ($data['license_key'] ? $data['license_key'] : 'N/A') . '</a>';
	$nestedData['license_end_date'] = "<a  style='display:block;color:" . $color . "' href='view_order.php?id=" . $data['id'] . "'>" . $ed . '</a>';
	$nestedData['quantity'] = "<a style='display:block;color:" . $color . "' href='view_order.php?id=" . $data['id'] . "'>" . $data['quantity'] . '</a>';
	$nestedData['company_name'] = "<a style='display:block;color:" . $color . "' href='view_order.php?id=" . $data['id'] . "'>" . $data['company_name'] . '</a>';
	...
	$nestedData['caller'] = getSingleresult("select name from callers where id='" . $data['caller'] . "'") . ($data['allign_to'] ? '<br><span style="font-size:8px">(' . getSingleresult("select name from users where id=" . $data['allign_to']) . ')</span>' : '');
	$ids2 = "'but2" . $data['id'] . "'";
