...
if($requestData['upsell'] == '1' || $requestData['upsell'] == '0')
{
	...
	$urlCond.= "&upsell=".$requestData['upsell'];
}
...
while ($stage=db_fetch_array($stagesA)) {  
	...
	$maker_lab = getSingleResult("SELECT SUM(t.total_price) AS total_sum FROM orders o JOIN tbl_lead_product_opportunity t ON o.id = t.lead_id WHERE t.main_product_id IN (8) AND o.stage = '" . $stage['stage'] . "' AND o.agreement_type = 'Renewal' AND t.status!=0 AND o.is_opportunity = 1 AND t.financial_year_start = '$fy_year' $dat ");

	$stop_motion = getSingleResult("SELECT SUM(t.total_price) AS total_sum FROM orders o JOIN tbl_lead_product_opportunity t ON o.id = t.lead_id WHERE t.product=34 AND o.stage = '" . $stage['stage'] . "' AND o.agreement_type='Renewal' AND t.status!=0 AND o.is_opportunity = 1 and t.financial_year_start = '$fy_year' $dat ");

	$cubo = getSingleResult("SELECT SUM(t.total_price) AS total_sum FROM orders o JOIN tbl_lead_product_opportunity t ON o.id = t.lead_id WHERE t.product=35 AND o.stage = '" . $stage['stage'] . "' AND o.agreement_type='Renewal' AND t.status!=0 AND o.is_opportunity = 1 and t.financial_year_start = '$fy_year' $dat ");

	$foundation = getSingleResult("SELECT SUM(t.total_price) AS total_sum FROM orders o JOIN tbl_lead_product_opportunity t ON o.id = t.lead_id WHERE t.product=36 AND o.stage = '" . $stage['stage'] . "' AND o.agreement_type='Renewal' AND t.status!=0 AND o.is_opportunity = 1 and t.financial_year_start = '$fy_year' $dat ");
	
	$advanced = getSingleResult("SELECT SUM(t.total_price) AS total_sum FROM orders o JOIN tbl_lead_product_opportunity t ON o.id = t.lead_id WHERE t.product=37 AND o.stage = '" . $stage['stage'] . "' AND o.agreement_type='Renewal' AND t.status!=0 AND o.is_opportunity = 1 and t.financial_year_start = '$fy_year' $dat ");
	
	$iot = getSingleResult("SELECT SUM(t.total_price) AS total_sum FROM orders o JOIN tbl_lead_product_opportunity t ON o.id = t.lead_id WHERE t.product=38 AND o.stage = '" . $stage['stage'] . "' AND o.agreement_type='Renewal' AND t.status!=0 AND o.is_opportunity = 1 and t.financial_year_start = '$fy_year' $dat ");
	
	$robotics = getSingleResult("SELECT SUM(t.total_price) AS total_sum FROM orders o JOIN tbl_lead_product_opportunity t ON o.id = t.lead_id WHERE t.product=39 AND o.stage = '" . $stage['stage'] . "' AND o.agreement_type='Renewal' AND t.status!=0 AND o.is_opportunity = 1 and t.financial_year_start = '$fy_year' $dat ");
	
	$creality = getSingleResult("SELECT SUM(t.total_price) AS total_sum FROM orders o JOIN tbl_lead_product_opportunity t ON o.id = t.lead_id WHERE t.product=40 AND o.stage = '" . $stage['stage'] . "' AND o.agreement_type='Renewal' AND t.status!=0 AND o.is_opportunity = 1 and t.financial_year_start = '$fy_year' $dat ");
