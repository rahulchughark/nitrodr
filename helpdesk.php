...
if ($id > 0 && $school_id > 0) {
    ...
    $token = $dataObj->getKMSAuthToken($_SESSION['name']);    
    // $school_id = 505;
    $tickets_data = $dataObj->getSchoolHelpdeskDetails($token, $school_id,$_SESSION['name']);
}


// Prepare tickets array for table display
$tickets = [];
if (!empty($tickets_data) && is_array($tickets_data)) {
    foreach ($tickets_data as $ticket) {
        if (!empty($ticket['q_id'])) {
            $tickets[] = [
                'id' => $ticket['q_id'],
                'subject' => $ticket['subject'],
                'query_raised_by' => $ticket['query_raised_by'] ? $ticket['query_raised_by'] : 'NA',
                'created_date' => $ticket['created_date'],
                'closed_time' => $ticket['closed_time'],
                'closed_by_name' => $ticket['closed_by_name'] ? $ticket['closed_by_name'] : 'NA',
                                ...
                                <div class="col">
                                    <div class="media bredcrum-title"><img class="d-flex mr-3 rounded-circle avatar-xs" src="images/title-icon.png" alt=" ">
                                        <div class="media-body">
                                            <small class="text-muted">Home > Helpdesk</small>
                                            <h4 class="font-size-14 m-0 mt-1">Helpdesk</h4>
                                        </div>
                                        <!-- <a href="#" id="addCopy">
                                ...
                                <div class="col-12">
                                        <?php if (!empty($tickets) && count($tickets) > 0): ?>
                                            <div class="table-responsive mt-3">
                                                <!-- id="ticketsTable" -->
                                                <table class="table" id="ticketsTable1">
                                                    <thead>
                                                        <tr>
                                                            <th>S.no</th>
                                                            <th>Ticket ID</th>
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <?php foreach ($tickets as $ticket): ?>
                                                            <tr>
                                                                ...
                                                                <td><?= $ticket['query_raised_by'] ?></td>
                                                                <td><?= $ticket['closed_by_name'] ?></td>
                                                                <td><?= $ticket['status_name'] ?></td>
                                                                <td><?= date('d M-Y, h:i:s', strtotime($ticket['created_date'])) ?></td>
                                                                <td><?= date('d M-Y, h:i:s', strtotime($ticket['closed_time'])) ?></td>
                                                                <td>
                                                                    <button style="background:#44a2d2" class="btn btn-sm btn-info view-ticket" 
                                                                            data-json='<?= htmlspecialchars(json_encode($ticket), ENT_QUOTES) ?>'>
                                                                        <i class="fa fa-eye"></i>
                                                                    </button>
                                                                </td>
                                                            </tr>
...
$(document).on("click", ".view-ticket", function () {
    ...
    $.post("view-ticket.php", { ticket: ticketData }, function (response) {
