    ...
    <div class="modal-content">
      ...
      <?php /*?>  <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Goals for <?=getSingleresult("select name from partners where id=".$_POST['pid'])?></h4><?php */?>
         ...
         <h4 class="modal-title">Import Renewal Leads</h4>
         <button type="button" class="close" data-dismiss="modal">&times;</button>
        
      </div>
      <div class="modal-body">
      Please be carefull while uploading file and ensure that all the below condtions are followed.
      <ul>
      <li>Keep colum order same as attached csv format</li><!-- 
      <li>Fill real Partner id (No SFDC ID) in the colum "team_id"</li> -->
      <li>Avoid Special Charecters such as "Apostrophe(')"</li>
      <li>If you want to input more than one User Name,Email,Contact And Designation then ensure value count should be equal in every column and use separated comma(,) to input more that one values</li>
      </ul>
      <a href="uploads/renwal_import_format.csv" download>Download CSV Format</a>
