...
function personalReport_TableData($table, $field1, $field2, $field3)
{
  ...
  WHERE 1=1 GROUP BY o.id ORDER By o.id Desc LIMIT " . $field2 . " ," . $field3 . "");
...
function massLead_TableData($table, $field1, $field2)
{
  ...
  $select_query = db_query("SELECT o.r_user,o.created_date,o.lead_type,o.r_name,o.license_type,o.industry,o.caller,o.id,o.status,o.code,o.quantity,o.company_name,o.eu_email,o.eu_mobile,o.close_time,o.stage,cl.name as caller,i.name as industry FROM $table as o
  LEFT JOIN callers as cl ON o.caller = cl.id
  LEFT JOIN industry as i ON o.industry = i.id
  WHERE 1=1 and o.status!='Cancelled' and o.status!='Undervalidation' GROUP BY o.id ORDER By o.id Asc LIMIT " . $field1 . " ," . $field2 . "");
  return $select_query;
}
...
function massLead_DataWithConditions($table, $field1, $field2, $field3)
{
  ...
  $select_query = db_query("SELECT o.r_user,o.created_date,o.lead_type,o.r_name,o.license_type,o.industry,o.caller,o.id,o.status,o.code,o.quantity,o.company_name,o.eu_email,o.eu_mobile,o.close_time,o.stage,cl.name as caller,i.name as industry,p.product_id,p.product_type_id FROM $table as o
  LEFT JOIN callers as cl ON o.caller = cl.id
  LEFT JOIN industry as i ON o.industry = i.id
  left join tbl_lead_product as p on o.id=p.lead_id
  WHERE 1 $field1 and o.status!='Cancelled' and o.status!='Undervalidation'
...
function get_tree()
{
  ...
  foreach (db_query("select * from tbl_module where parentId='0' order by setOrder ASC") as $res1) {
    $spacing = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
    $cat_tree_arr[] = array('id' => $res1['id'], 'url' => $res1['url'], 'icon' => $res1['icon'], 'setOrder' => $res1['setOrder'], 'parentId' => $res1['parentId'], 'name' => $res1['name'] . $spacing . $res1['user_type'], 'status' => $res1['status'], 'user_type' => $res1['user_type']);
...
function update_roles_permission($id, $field1, $field2, $field3, $field4, $field5, $field6, $field7)
{
  if ($dbcon2 == '') {
    if (!isset($GLOBALS['dbcon'])) {
      connect_db();
  ...
  $update_query = db_query("update tbl_role_permission set edit_log='" . $field1 . "',edit_lead='" . $field2 . "',edit_stage='" . $field3 . "',edit_date='" . $field4 . "',edit_ownership='" . $field5 . "',edit_status='" . $field6 . "',edit_review_log='" . $field7 . "' where role_id=" . $id);
  return $update_query;
}
...
function insert_roles_permission($field1, $field2, $field3, $field4, $field5, $field6, $field7, $field8)
{
  if ($dbcon2 == '') {
    if (!isset($GLOBALS['dbcon'])) {
      connect_db();
  ...
  $query = db_query("insert into tbl_role_permission(role_id,edit_log,edit_lead,edit_stage,edit_date,edit_ownership,edit_status,edit_review_log) values('" . $field1 . "','" . $field2 . "','" . $field3 . "','" . $field4 . "','" . $field5 . "','" . $field6 . "','" . $field7 . "','" . $field8 . "')");
  return $query;
}
...
function get_campaign_data()
{
  ...
  $res = db_query("select c.*,u.name as user,tp.product_name from campaign as c left join users as u on c.created_by=u.id left join tbl_product as tp on c.product_id=tp.id");

  return $res;
}

function update_campaign($field1, $field2, $field3, $field4, $field5, $field6, $field7, $field8)
{
  if ($dbcon2 == '') {
    if (!isset($GLOBALS['dbcon'])) {
      connect_db();
  ...
  $res = db_query("UPDATE `campaign` set `name`='" . $field1 . "',`description`='" . $field2 . "',`status`='" . $field3 . "',`start_date`='" . $field4 . "',`end_date`='" . $field5 . "',`created_by`='" . $field6 . "',`product_id`='" . $field8 . "' where id=" . $field7);
...
function insertLeadData($table, $field4, $field5, $field6, $field7, $field8, $field9, $field10, $field11, $field12, $field13, $field14, $field15, $field16, $field17, $field18, $field19, $field20, $field21, $field22, $field23, $field24, $field25, $field26, $field27, $field28, $field29, $field30, $field31, $field32, $field33, $field34, $field35, $field36, $field37, $field38, $field39, $field40,$field41)
{
  ...
  $res = db_query("INSERT INTO `$table`(`r_name`, `r_email`, `r_user`,`source`, `lead_type`, `company_name`, `parent_company`, `landline`,region, `industry`,sub_industry, `address`, `pincode`, `state`, `city`, `country`, `eu_name`, `eu_email`, `eu_landline`, `department`, `eu_mobile`, `eu_designation`, `eu_role`, `visit_remarks`,`account_visited`,`confirmation_from`, `license_type`, `quantity`, `created_by`, `team_id`, `status`,user_attachement,os,version,runrate_key,partner_close_date,campaign_type,association_name,validation_type) VALUES ('" . $field37 . "','" . $field38 . "','" . $field39 . "','" . $field4 . "','" . $field5 . "','" . htmlspecialchars($field6, ENT_QUOTES) . "','" . htmlspecialchars($field7, ENT_QUOTES) . "','" . $field8 . "','" . $field9 . "','" . $field10 . "','" . $field11 . "','" . htmlspecialchars($field12, ENT_QUOTES) . "','" . $field13 . "','" . $field14 . "','" . $field15 . "','" . $field16 . "','" . $field17 . "','" . $field18 . "','" . $field19 . "','" . $field20 . "','" . $field21 . "','" . $field22 . "','" . $field23 . "','" . htmlspecialchars($field24, ENT_QUOTES) . "','" . $field31 . "','" . $field32 . "','" . $field33 . "','" . $field25 . "','" . $field26 . "','" . $field27 . "','Pending','" . $field28 . "','" . $field34 . "','" . $field35 . "','" . $field36 . "','" . $field29 . "','" . $field30 . "','" . $field40 . "','" . $field41 . "')");
...
function leadViewData($table, $field1, $field2)
{
  ...
  $query = db_query("select o.*,c.name as campaign,tp.*,p.product_name,tpp.product_type,tpp.id as type_id from $table as o left join campaign as c on o.campaign_type = c.id left join tbl_lead_product as tp on o.id=tp.lead_id left join tbl_product as p on tp.product_id=p.id left join tbl_product_pivot as tpp on tp.product_type_id=tpp.id where 1 " . $field1 . " and o.id=" . $field2);
...
function selectEditDetails($field1, $field2, $field3 = NULL)
{
  ...
  $query = db_query("select o.*,tp.*,p.product_name,p.id as productId,tpp.product_type from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join tbl_product as p on tp.product_id=p.id left join tbl_product_pivot as tpp on tp.product_type_id=tpp.id where o.id=" . $field1 . $condition);
...
*/
function newDR_KRA_BO($team_id, $date_from, $date_to, $created_by)
{
  $query = getSingleresult("select count(distinct(o.id)) from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join activity_log on activity_log.pid=orders.id where tp.product_type_id in (1,2) and o.license_type='Commercial' and o.lead_type='LC' and o.team_id='" . $team_id . "' and o.validation_type='profiling_validation' and activity_log.call_subject='Profiling Call' and date(activity_log.created_date)>='" . $date_from . "' and o.created_by in (" . $created_by . ") and date(activity_log.created_date)<='" . $date_to . "'");
...
function rawLeads_BO($team_id, $date_from, $date_to, $created_by)
{
...
function rawLeads_SALES($team_id, $created_by, $date_from, $date_to)
{
  $query = getSingleresult("select count(distinct(id)) from raw_leads where (product_type_id=1 or product_type_id=2) and team_id='" . $team_id . "' and created_by='" . $created_by . "' and date(created_date)>='" . $date_from . "' and date(created_date)<='" . $date_to . "'");
  return $query;
}
...
function tc_approved($team_id, $created_by, $date_from, $date_to)
{
  $query = getSingleresult("select count(distinct(o.id)) from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id where (tp.product_type_id=1 or tp.product_type_id=2) and o.team_id='" . $team_id . "' and o.created_by='" . $created_by . "' and date(o.approval_time)>='" . $date_from . "' and date(o.approval_time)<='" . $date_to . "' and o.status='Approved'");
  return $query;
}
...
*/
function newDR_KRA_BO_dash($team_id, $date_from, $date_to, $created_by)
{
  $query = getSingleresult("select count(distinct(o.id)) from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id where (tp.product_type_id=1 or tp.product_type_id=2) and o.license_type='Commercial' and o.status='Approved' and o.team_id='" . $team_id . "' and MONTH(o.approval_time)=" . $date_from . " and o.created_by in (" . $created_by . ") and YEAR(o.approval_time)=" . $date_to . " ");
...
function rawLeads_BO_dash($team_id, $date_from, $date_to, $created_by)
{
  $query = getSingleresult("select count(distinct(id)) from raw_leads where (product_type_id=1 or product_type_id=2) and team_id='" . $team_id . "' and created_by in (" . $created_by . ") and MONTH(created_date)=" . $date_from . " and YEAR(created_date)=" . $date_to . "");
  return $query;
}
...
{
  $query = getSingleresult("SELECT count(distinct(activity_log.id)) FROM  activity_log left join orders on activity_log.pid=orders.id left join tbl_lead_product as tp on orders.id=tp.lead_id where (tp.product_type_id=1 or tp.product_type_id=2) and orders.team_id='" . $team_id . "' and orders.license_type='Commercial' and MONTH(activity_log.created_date)=" . $date_from . " and YEAR(activity_log.created_date)=" . $date_to . " and activity_log.added_by in (" . $created_by . ") and activity_log.call_subject in ('Fresh Call','Follow-up Call') and orders.id!='' and activity_log.activity_type='Lead' and activity_log.is_intern=0 and activity_log.call_subject!=''");

  return $query;
}
function totalLog_lapsed_BO_dash($team_id, $date_from, $date_to, $created_by)
{
  $query = getSingleresult("SELECT count(distinct(activity_log.id)) FROM  activity_log left join lapsed_orders as l on activity_log.pid=l.id left join tbl_lead_product as tp on l.id=tp.lead_id where (tp.product_type_id=1 or tp.product_type_id=2) and l.team_id='" . $team_id . "' and l.license_type='Commercial' and MONTH(activity_log.created_date)=" . $date_from . " and YEAR(activity_log.created_date)=" . $date_to . " and activity_log.added_by in (" . $created_by . ") and activity_log.call_subject in ('Fresh Call','Follow-up Call') and l.id!='' and activity_log.activity_type='Lead' and activity_log.is_intern=0 and activity_log.call_subject!=''");

  return $query;
}
function totalLog_raw_BO_dash($team_id, $date_from, $date_to, $created_by)
{
  $query = getSingleresult("SELECT count(distinct(activity_log.id)) FROM  activity_log left join raw_leads as o on activity_log.pid=o.id where (o.product_type_id=1 or o.product_type_id=2) and o.team_id='" . $team_id . "' and  activity_log.added_by in (" . $created_by . ") and MONTH(activity_log.created_date)=" . $date_from . " and YEAR(activity_log.created_date)=" . $date_to . " and activity_log.call_subject in ('Fresh Call','Follow-up Call') and activity_log.activity_type='Raw' and o.id!='' and activity_log.is_intern=0 and activity_log.call_subject!=''");
  return $query;
}
...
function rawLeads_SALES_dash($team_id, $created_by, $date_from, $date_to)
{
...
function tc_approved_dash($team_id, $created_by, $date_from, $date_to)
{
...
{
  $query = "select orders.r_name,orders.r_user,orders.lead_type,orders.quantity,orders.company_name,orders.eu_email,orders.id,orders.eu_mobile,orders.team_id,orders.stage,lead_review.is_review,lead_review.added_date,p.product_id,p.product_type_id,orders.created_date from orders join lead_review on orders.id=lead_review.lead_id left join tbl_lead_product as p on orders.id=p.lead_id where 1=1  and orders.license_type='Commercial' and orders.team_id='" . $field1 . "' " . $field2 . " GROUP BY orders.id";
...
function partnerReviewLeadsSearch($field1)
{
  ...
  $query = "select orders.r_name,orders.r_user,orders.lead_type,orders.quantity,orders.company_name,orders.eu_email,orders.id,orders.eu_mobile,orders.team_id,orders.stage,lead_review.is_review,lead_review.added_date,p.product_id,p.product_type_id,orders.created_date from orders join lead_review on orders.id=lead_review.lead_id left join tbl_lead_product as p on orders.id=p.lead_id where 1=1  and orders.license_type='Commercial' and orders.team_id='" . $field1 . "'";
...
{
  $query = getSingleresult("SELECT count(distinct(activity_log.id)) FROM  activity_log left join raw_leads as o on activity_log.pid=o.id where (o.product_type_id=1 or o.product_type_id=2) and o.team_id='" . $team_id . "' and date(activity_log.created_date)>='" . $date_from . "' and date(activity_log.created_date)<='" . $date_to . "' and activity_log.added_by in ('" . $created_by . "') and activity_log.call_subject in ('Fresh Call','Follow-up Call') and activity_log.activity_type='Raw' and o.id!='' and activity_log.is_intern=0");

  return $query;
}
function logISS_leadadmin($team_id, $date_from, $date_to, $created_by)
{
  $query = getSingleresult("SELECT count(distinct(activity_log.id)) FROM  activity_log left join orders on activity_log.pid=orders.id left join tbl_lead_product as tp on orders.id=tp.lead_id where (tp.product_type_id=1 or tp.product_type_id=2) and orders.team_id='" . $team_id . "' and  activity_log.added_by in ('" . $created_by . "') and orders.license_type='Commercial' and date(activity_log.created_date)>='" . $date_from . "' and date(activity_log.created_date)<='" . $date_to . "' and activity_log.call_subject in ('Fresh Call','Follow-up Call') and activity_log.activity_type='Lead' and orders.id!='' and activity_log.is_intern=0");
...
{
  $query = getSingleresult("SELECT count(distinct(activity_log.id)) FROM  activity_log left join lapsed_orders as l on activity_log.pid=l.id left join tbl_lead_product as tp on l.id=tp.lead_id where (tp.product_type_id=1 or tp.product_type_id=2) and l.team_id='" . $team_id . "' and  activity_log.added_by in ('" . $created_by . "') and l.license_type='Commercial' and date(activity_log.created_date)>='" . $date_from . "' and date(activity_log.created_date)<='" . $date_to . "' and activity_log.call_subject in ('Fresh Call','Follow-up Call') and activity_log.activity_type='Lead' and l.id!='' and activity_log.is_intern=0");
  return $query;
}
...
function activityLogs($field1, $field2, $field3, $field4,$field5)
{
  ...
  $res = db_query("insert into activity_log (pid,description,activity_type,call_subject,added_by,action_plan) values ('" . $field1 . "','" . htmlspecialchars($field2, ENT_QUOTES) . "','Lead','" . $field3 . "','" . $field4 . "','" . $field5 . "')");
...
{
  $query = getSingleresult("select count(DISTINCT(o.id)) as leads from orders as o left join tbl_lead_product as p on o.id=p.lead_id where o.team_id='" . $team_id . "' and date(o.approval_time) BETWEEN  '" . $dat1 . "' and '" . $dat2 . "' and p.product_type_id in (1,2) and o.status='Approved' and o.dvr_flag=0 and o.iss is NULL  and o.id not in (" . $lead_id . ")");
  return $query;
}

function dvrReportsLCQualified($team_id,$dat1,$dat2,$lead_id)
{
  $query = getSingleresult("select count(DISTINCT(o.id)) as leads from orders as o left join tbl_lead_product as p on o.id=p.lead_id left join lead_modify_log lm on o.id=lm.lead_id where o.team_id='" . $team_id . "' and date(o.approval_time)>='" . $dat1 . "' and date(o.approval_time)<='" . $dat2 . "' and o.lead_type='LC' and o.status='Approved' and p.product_type_id in (1,2) and o.dvr_flag=0 and o.iss is NULL and o.id not in (" . $lead_id . ") and lm.previous_name not in ('BD','Incoming','LC')");
  return $query;
}
function dvrReportsBDQualified($team_id,$dat1,$dat2,$lead_id)
{
  $query = getSingleresult("select count(DISTINCT(o.id)) as leads from orders as o left join tbl_lead_product as p on o.id=p.lead_id where o.team_id='" . $team_id . "' and date(o.approval_time)>='" . $dat1 . "' and date(o.approval_time)<='" . $dat2 . "' and o.lead_type='BD' and o.status='Approved' and p.product_type_id in (1,2) and o.dvr_flag=0 and o.iss is NULL and o.id not in (" . $lead_id . ")");
  return $query;
}
function dvrReportsIncomingQualified($team_id,$dat1,$dat2,$lead_id)
{
  $query = getSingleresult("select count(DISTINCT(o.id)) as leads from orders as o left join tbl_lead_product as p on o.id=p.lead_id where o.team_id='" . $team_id . "' and date(o.approval_time)>='" . $dat1 . "' and date(o.approval_time)<='" . $dat2 . "' and o.lead_type='Incoming' and o.status='Approved' and p.product_type_id in (1,2) and o.dvr_flag=0 and o.iss is NULL and o.id not in (" . $lead_id . ")");
  return $query;
}
function dvrReportsConvertedData($team_id,$dat1,$dat2,$lead_id,$dat)
{
  $query = db_query("select * from (select count(DISTINCT(o.id)),o.id from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join lead_modify_log lm on o.id=lm.lead_id where tp.product_type_id in (1,2) and  (date(lm.created_date) BETWEEN  '" . $dat1 . "' and '" . $dat2 . "') and o.team_id='" . $team_id . "' and o.license_type='Commercial' and lm.type='Lead Type' and lm.previous_name in ('BD','Incoming') and lm.modify_name='LC' $dat GROUP BY o.id) t1 INNER JOIN (select count(DISTINCT(o.id)),o.id from orders as o left join tbl_lead_product as tp on o.id=tp.lead_id left join lead_modify_log lm on o.id=lm.lead_id where tp.product_type_id in (1,2) and  (date(lm.created_date) BETWEEN  '" . $dat1 . "' and '" . $dat2 . "') and o.license_type='Commercial' and o.team_id='" . $team_id . "' and lm.type='Caller' and lm.previous_name ='' and lm.modify_name in ('" . $lead_id . "') $dat GROUP BY o.id) t2 on t1.id=t2.id");
  return $query;
}