...
$product_result = $dataObj->fetchOrderAttachments($orderID,'invoice_attachments',$is_group,$group_name);

$countData = db_num_array($product_result);

$getEMIs = $dataObj->getInvoiceEmiByOrderId($orderID);
$pageLabel = $type == 1 ? "Invoice Edit" : "Invoice View";
                                    ...
                                    while ($data = db_fetch_array($product_result)) {
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <?php 
                                                $pi_label = 'N/A';
                                                if ($data['parent_pi_id']) {
                                                    if (!empty($data['parent_pi_name']) && !empty($data['parent_pi_amount'])) {
                                                        $pi_label = htmlspecialchars($data['parent_pi_name']) . " (₹" . number_format($data['parent_pi_amount'], 2) . ")";
                                                    } else {
                                                        $pi_label = htmlspecialchars($data['parent_pi_attachment_name']) . ' <small class="text-muted">(a. name)</small>';
                                                    }
                                                }
                                                echo $pi_label;
                                                ?>
                                                ...
                                                <div class="d-flex align-items-center justify-content-center">
                                                    <div class="upload-btn">
                                                        <?php if ($type == 1): ?>
                                                            ...
                                                            <label class="btn btn-primary upload-btn<?= $data['attachment_id'] ?> mb-0 py-1"
                                                                for="Upload_<?= $data['attachment_id']; ?>">
                                                                <i class="mdi mdi-upload" style="font-size: 18px"></i> 
                                                            </label>
                                                        <?php endif; ?>
...
<script>
    ...
    function uploadAttachment(attachmentId, input, master_lead_id) {
        $(".upload-btn" + attachmentId)
            ...
            success: function (response) {
                try {
                    ...
                    if (res.status === 'success') {
                        ...
                        invoice_attach_edit(master_lead_id);
                    } else {
