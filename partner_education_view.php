...
if ($_POST['new_user']) {
    ...
    redir("partner_education_view.php?id=" . $_POST['id'], true);
}
...
if ($_POST['partner_close_date'] && !$_POST['stage']) {
    ...
    redir("education_leads_partner.php?update=success", true);
}
...
if ($_POST['submit_dvr']) {
    $res =  db_query("insert into lead_modify_log(`lead_id`,type,`previous_name`,`modify_name`,`created_date`,`created_by`)values('" . $_REQUEST['id'] . "','Education','Education','DVR',now(),'" . $_SESSION['user_id'] . "')");
...
if ($_POST['remarks'] || $_POST['call_subject']) {
    ...
    $res = db_query("insert into activity_log (pid,description,activity_type,call_subject,added_by,data_ref) values ('" . $_POST['pid'] . "','" . htmlspecialchars($_POST['remarks'], ENT_QUOTES) . "','Education','" . $_POST['call_subject'] . "','" . $_SESSION['user_id'] . "',1)");
...
if ($_REQUEST['id']) {
    ...
    redir("education_leads_partner.php", true);
}
?>
<!-- ============================================================== -->
...
<div class="card mb-0 pt-2 shadow-none">		  
                              <div class="card-header" id="headingOne3">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?= $r_user ?> <?php if (($created_by == $_SESSION['user_id']) || $_SESSION['user_type'] == 'MNGR') { ?> <button class="btn1 btn-primary" onclick="change_user('<?= $_GET['id'] ?>','<?= $_SESSION['team_id'] ?>')">Change</button> <?php } ?>
 ...
 <div class="card mb-0 pt-2 shadow-none">
  <div class="card-header" id="headingOne7">
                                ...
                                <?php if ($user_attachement && $user_attachement != '' && strpos($user_attachement, ".") ) { ?>
                                    ...
                                    <?php if($data['status'] == 'Undervalidation' && $data['status'] == 'Cancelled') { ?>
                                    <tr>
                                        <td>Reason</td>
                                        <td>
                                            <?= ($reason ? $reason : 'N/A') ?>
                                   ...
                                   <?php } ?>
                                    ...
                                    <button id="sub_btn" style="display:none; position: relative;" type="submit" name="save" onclick="" class="btn btn-primary">Submit as DVR</button>
                                        </form>
   ...
   <div class="card mb-0 pt-2 shadow-none">
                                        ...
                                        <div class="button-items1 mt-2">
                                            ...
                                            <?php if ($data['status'] != 'Approved' && $userId == $_SESSION['user_id']) { ?>
