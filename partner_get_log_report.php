...
if ($requestData['users']) {
   $cond .= " and o.r_user  in ('" . $requestData['users'] . "')"; 
}
...
$sql = "select o.r_user,o.r_name,o.code,o.company_name,o.eu_email,o.eu_mobile,o.id,o.status,o.approval_time,o.campaign_type,o.iss,o.lead_type,o.license_type,o.stage,o.team_id,o.caller,o.created_by,o.quantity FROM orders as o left join activity_log as activity_log on o.id = activity_log.pid left join industry as i on o.industry=i.id where 1 and o.license_type='Commercial' and activity_log.is_intern=0 and o.team_id='".$teamId."' " . $cond . $dat . $activity." GROUP BY o.id ";
...
while ($data = db_fetch_array($query)) {  // preparing an array
	...
	$nestedData[] = "<a style='display:block;color:" . $color . "' href='partner_view.php?id=" . $data['id'] . "'>" . $data['r_user'] . '</a>';
	$nestedData[] = "<a style='display:block;color:" . $color . "' href='partner_view.php?id=" . $data['id'] . "'>" . $data['company_name'] . '</a>';

// 	if ($_SESSION['user_type'] == 'OPERATIONS' || $_SESSION['user_type'] == 'SUPERADMIN' || $_SESSION['user_type'] == 'REVIEWER' || $_SESSION['user_type'] == 'ADMIN') {
		$nestedData[] = "<a style='display:block;color:" . $color . "' href='partner_view.php?id=" . $data['id'] . "'>" . $data['lead_type'] . '</a>';
// 	}
	$nestedData[] = "<a style='display:block;color:" . $color . "' href='partner_view.php?id=" . $data['id'] . "'>" . getSingleresult("select name from callers where id='" . $data['caller'] . "'") . '</a>';
