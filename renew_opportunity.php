...
$financialYears = [
    ...
    ($financialYearStart + 1) . "-" . ($financialYearEnd + 1)
];
$fncYear = getSingleresult("SELECT DISTINCT(financial_year_start) AS financial_year,financial_year_end FROM tbl_lead_product_opportunity WHERE lead_id = ".$_POST['pid']." ORDER BY financial_year desc limit 1");
// print_r($fncYear);die;
$query = db_query("SELECT DATE_FORMAT(created_at, '%Y-%m-%d') AS purchase_day,financial_year_start AS financial_year,tbl_lead_product_opportunity.* FROM tbl_lead_product_opportunity WHERE lead_id = ".$_POST['pid']." AND financial_year_start = '".$fncYear."' ORDER BY purchase_day ASC");
$countProduct = $query->num_rows;
if($countProduct==0){
    $parentID = getSingleresult("SELECT parent_id from orders where id=".$_POST['pid']);
...
<div class="modal-dialog modal-dialog-centered modal-xxl modal-dialog-scrollablel">
    <div class="modal-content" >
        <div class="modal-header">
            <h5 class="modal-title align-self-center mt-0" id="exampleModalLabel">Renew Opportunity</h5>
            ...
            <form id="opportunityForm" action="#" method="post" class="form p-t-20" enctype="multipart/form-data">
                <div class="row">
                    ...
                    <?php while($data = db_fetch_array($query)){     
                        $grand_total_price+=$data['total_price'];
                        $i++;
                        ...
                        <div class="col-md-2 mb-2"  id="productCode1">
                            ...
                            <div class="col-md mb-2">
                                <label class="control-label">Is Upsell</label>
                            ...
                            <input name="original_price[]" type="hidden" class="form-control original-sales-price" value="<?= ($data['original_sales_price'] != '' && $data['original_sales_price'] != 0 && $data['original_sales_price'] != null )? $data['original_sales_price'] : $data['unit_price'] ?>">
                        ...
                        <div class="col-md-12 mb-2">
                            ...
                            <input id="grand_total" name="grand_total" type="number" class="form-control" value="<?= $grand_total_price ?>" readonly />
                        </div>
                        <div class="col-md-12 mb-2">
                ...
                <input type="hidden" name="is_renew" value="yes" />
                            ...
                            if (data.success) {
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        window.location.href = "renewal_leads_admin.php";
                                    }
                                });
                            } else {
